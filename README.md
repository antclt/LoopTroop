...
`npm run dev` starts the local LoopTroop stack and runs the startup preflight. The preflight can refresh the local OpenCode CLI, sync direct npm dependencies, and run safe audit remediation during local development. Routine startup summarizes maintenance once, including held dependency and audit package counts; use `LOOPTROOP_DEV_VERBOSE=1 npm run dev` for held package names, next eligible times, audit finding notes, npm install output, service-plan details, and port diagnostics.
