...
| `PREPARING_EXECUTION_ENV` | The approved setup plan is executed in a constrained way. Missing required tooling is provisioned under ticket-owned temp roots before blocking, failed launcher setup must include `tool_requirements.provisioning_attempts` evidence for distinct strategies or a no-safe-path reason, and the goal is a reusable runtime profile rather than general project mutation. Ready setup results fail if declared wrappers are unusable, `tooling_probe_commands` fail or are missing for declared command families, or committable project changes remain; untracked generated/local outputs proceed as cautions with `.gitignore` suggestions. | Execution setup profile with tooling probes, runtime wrapper artifacts, setup logs, setup diagnostics. | `cancel` | Success enters `CODING`. |
