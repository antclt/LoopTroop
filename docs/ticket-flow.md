...
- **`GENERATING_QA_CHECKLIST`:** “LoopTroop is preparing a human-facing Manual QA checklist from approved context and focused implementation evidence, with generation status, logs, and the produced checklist artifact available in the phase workspace.” It is automation-only: LoopTroop resolves final-test effects, creates a clean local checkpoint/baseline, reserves `vN`, generates one strict tagged YAML checklist with the locked implementer, validates stable PRD refs, computes advisory coverage in code, and advances automatically. The preparation workspace follows other artifact-producing phases: it shows a concise live status, a clickable generated-checklist artifact, and the phase log, while the next status owns the interactive form. It never starts or controls the application. Generation, validation, or checkpoint failure enters recoverable `BLOCKED_ERROR` and retry reuses the reservation/valid artifacts.
- **`WAITING_MANUAL_QA`:** “LoopTroop is waiting for user-run verification in an autosaved Manual QA checklist: items start Pending, Pass and Waive require no evidence, file changes settle before completion, and Submit or Skip finishes the round.” The user runs and controls the app. Pending is the first/default choice and stays field-free until another result is selected; required items must be resolved for Submit, Pass/Waive need no evidence, Fail needs an observation, and Pass notes/waiver reasons are optional. PRD coverage is collapsed by default. Improvements are edited inline with collapsed context, final-description, and evidence/provenance previews. Failure groups are multi-select item number/title buttons; group drafts may include any item, but Submit identifies and blocks on every member not marked Fail. Results/evidence autosave with no Save button and show a relative last-save time plus exact hover timestamp. Extra evidence exposes Add link then Add files, successful uploads appear immediately, and only the first five entries show until expanded. Submit/Skip wait for active file mutations; Submit retains durable files, drops stale optional references, and uses item/title/filename diagnostics for remaining integrity errors. Submit creates reviewed improvements and grouped/ungrouped fix beads together through an idempotent journal. Pass, required waiver, or skip integrates; any Fail returns to Coding. **Skip Manual QA…** warns that no work will be created, bypasses normal item/group completeness checks, and archives every entered value read-only. Before submit/skip, audited workspace drift must be included in a checkpoint or discarded. Historical rounds stay selectable read-only.
