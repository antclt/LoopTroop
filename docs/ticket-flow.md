...
| `WAITING_PR_REVIEW` | Automation stops while you review the draft PR and decide whether to merge it or finish without merging. GitHub merge completion is verified against the remote base branch, and external merges can be detected and synchronized from here without modifying the local checkout. | Stable review gate, merge report after decision. | `merge`, `close_unmerged`, `cancel` | Merge or close-unmerged both advance to cleanup once remote verification succeeds; retries for already-merged PRs recheck the remote branch without a second merge. |
