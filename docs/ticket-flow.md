...
| Status | What happens here | Main outputs | User action | Normal exits |
| --- | --- | --- | --- | --- |
| `RUNNING_FINAL_TEST` | The main implementer generates the final-test plan under AI Response Timeout, then verifies the whole ticket result holistically with command execution governed by the execution timeout. Any files left dirty by the passing final test are classified through language-agnostic `file_effects` and recorded in a file-effects audit. | Final test artifact, file-effects audit, test results, retry diagnostics if needed. | `cancel` | Passing tests advance to integration. |
