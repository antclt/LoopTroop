...
- local bead commits are required when Git-visible project changes exist in any language or file extension; `.ticket/**`, `.looptroop/**`, execution-setup roots, and setup cache roots are kept local and excluded from commit capture, true no-op completions may finish without a commit, and remote push failures are warnings after a successful local commit
...
- common generated/local paths like `node_modules`, `dist`, `build`, `.env`, and log files are skipped only while untracked, with suggested `.gitignore` entries surfaced in warnings; if such paths are already tracked by the repository, bead changes to them remain committable
...
When setup provisions tooling, it writes `.ticket/runtime/execution-setup/env.sh` and `.ticket/runtime/execution-setup/run`. LoopTroop validates any declared wrapper by launching a no-op command through Node's current executable, then reruns every declared tooling probe using the same backend command execution path used by later phases. Final testing now reuses a validated setup wrapper automatically: if the model returns an unwrapped command and the setup profile declares a wrapper, LoopTroop executes the command through that wrapper and records both the original and effective command in the report. Execution setup retries clear stale profile and wrapper files but preserve `tool-cache`; a one-strategy provisioning failure can receive a small bounded persistence extension to try another safe strategy, while repeated-tooling early stop applies only after no-safe-path evidence or at least two distinct failed provisioning strategies.
