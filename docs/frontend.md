...
The Current Activity strip above phase and full log views is intentionally diagnostic-only after the first model activity. It shows waiting-for-first-activity and provider retry states before output starts, terminal timeout/empty-output diagnostics from trusted LoopTroop/OpenCode status rows, and an `Approaching timeout` warning only when prompt deadline metadata says the configured timeout is close. Model text, tool output, debug raw messages, and repository source snippets are never scanned for generic timeout words, so target-code symbols such as `ErrTimeout` or `fsWatcherTimeoutS` cannot produce a false workflow timeout banner.
