...
The `DEBUG` tab provides a complete view of every log line for the ticket. It loads `channel=all` on demand, which merges all three LoopTroop log files (`execution-log.jsonl`, `execution-log.debug.jsonl`, `execution-log.ai.jsonl`) plus OpenCode native server log lines filtered by the ticket's session IDs. All OpenCode SDK stream event types — including `part_removed` — are logged to the debug channel. OpenCode native logs are always written at DEBUG level (the managed server is always started with `--print-logs --log-level DEBUG`). Other tabs (`ALL`, `SYS`, `AI`, `ERROR`) are unaffected — they classify entries from structured `source`, `audience`, and `kind` fields regardless of which channel loaded them.
