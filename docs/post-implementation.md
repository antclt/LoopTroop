...
The user—not LoopTroop—starts and controls the application, follows prerequisites/actions, and records results. Pending is the first and default choice and shows no evidence or result-specific fields, only brief guidance to select Pass, Fail, Waive, or Improvement. Required items cannot remain Pending on Submit, while optional items may. Any Fail blocks integration, including an optional item, and requires an observation. A waiver reason and Pass notes are optional. Improvements are non-blocking, require a reviewed title/description/context, and each produces exactly one Normal-priority Draft ticket in the same project on final Submit.
