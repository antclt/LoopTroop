...
The user—not LoopTroop—starts and controls the application, follows prerequisites/actions, and records results. Pending is the first and default choice and shows no evidence or result-specific fields, only brief guidance to select Pass, Fail, Waive, or Improvement. Required items cannot remain Pending on Submit, while optional items may. Pass and Waive require no evidence; a waiver reason and Pass notes are optional. Any Fail blocks integration, including an optional item, and requires an observation. Improvements are non-blocking, require a reviewed title/description/context, and each produces exactly one Normal-priority Draft ticket in the same project on final Submit. Active evidence mutations finish before completion; Submit retains durable files, drops dangling optional references, and reports any remaining integrity issue with the item number/title and filename.
...
Failure merge groups are multi-select item buttons labeled with checklist number and title. A failed item may select any other item, even before that member is marked Fail, but Submit is blocked until every member is Fail and the validation message names each unresolved number/title. Improvement editing remains inline in the checklist item. Advisory PRD coverage, Manual QA context, final-description preview, and evidence/provenance preview all start collapsed.

Before Submit or Skip, the worktree is compared with the QA baseline. If running the app caused drift, the gate stays open until the user includes exactly audited files in a checkpoint or discards exactly those changes. Submission then uses an operation-typed journal with deterministic origins/action IDs so partial restarts cannot switch Submit into Skip, change the frozen draft, overwrite immutable results, or duplicate beads/improvement tickets. Submit promotes the frozen draft to `manual_qa_results`. **Skip Manual QA…** first warns that it creates no QA fix bead or Improvement ticket, then bypasses ordinary result, observation, and merge-group completeness checks. Skip preserves every entered result, note, grouping, Improvement draft, and evidence reference in the immutable draft plus skip receipt and summary; the archived data remains visible but cannot be edited later. A durable final summary repairs an incomplete journal and any missing compact summary/skip-receipt phase mirrors before replaying the workflow transition.
