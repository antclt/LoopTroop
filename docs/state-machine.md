...
| `WAITING_EXECUTION_SETUP_APPROVAL` | Review the readiness audit and approve any temporary workspace preparation with content-hash protection before any setup commands run. Manual setup-plan edits write user-edit receipts, and archived versions are read-only. |
