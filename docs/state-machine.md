...
| `CREATING_PULL_REQUEST` | Audits candidate files before remote side effects, rewrites the local candidate when evidence-backed exclusions are found, records ignored-file reasons and the net candidate diff, then drafts the PR title/body and pushes the final branch. |
| `WAITING_PR_REVIEW` | Review the draft pull request, final net diff, bead activity views, ignored-file audit, and test results, then choose Merge PR & Finish or Finish Without Merge. GitHub merge success is verified against the remote base branch, and ticket completion leaves the local checkout untouched. |
