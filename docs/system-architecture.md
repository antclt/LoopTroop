...
| --- | --- |
| Browser reload, close, or reconnect gap | REST state remains canonical; the browser log cache restores recent active log detail, SSE reconnect includes the last event id, replays buffered live events including the latest streaming upsert per log entry, and then refetches tickets, artifacts, bead state, interview state, and server logs |
