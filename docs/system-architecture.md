...
| User edits or regenerates setup plan during runtime setup | While still in `PREPARING_EXECUTION_ENV`, stop active runtime setup, archive the approved setup-plan attempt and runtime attempt, clear stale setup profile outputs while preserving the tool cache, return to `WAITING_EXECUTION_SETUP_APPROVAL`, and require approval again |
