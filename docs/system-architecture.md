...
| `.ticket/runtime/execution-log.ai.jsonl` | Persisted AI detail log stream for prompts, thinking, tool calls, and streamed model rows | Read by `/api/files/:ticketId/logs?channel=ai`; folded into AI/model log views; finalized assistant message parts for each completed prompt are also backfilled from `session.messages()` so multi-message tool-use detail history does not depend on a browser or live SSE stream being open |
