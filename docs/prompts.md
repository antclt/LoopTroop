...
| Manual QA checklist builder | `server/phases/manualQa/generator.ts` | `GENERATING_QA_CHECKLIST` | Fresh locked-main-implementer session / focused read-only inspection | focused read-only | Ticket details, approved PRD, selected bead fields, current final-test report, latest previous QA, targeted diff metadata | Produces one strict tagged YAML checklist with stable lineage inputs and full/partial PRD refs; coverage is computed in code from this response. |
...
- Manual QA generation prohibits raw whole-repository dumps and performs no second coverage prompt. Invalid PRD refs are schema failures, not advisory text. QA-fix coding prompts keep typed `qaOrigin` separate from retry notes; image-capable models receive every detected image evidence file as an OpenCode SDK file part, while non-images remain references and `references_only` is recorded when capability is false/unavailable.
