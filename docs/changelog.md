...
### Summary
- Reworked the Beads & Execution docs to match the implemented approval, setup handoff, retry/recovery, and artifact model, and lightly synchronized the Pre-Implementation setup-plan wording.
...
#### Changed
- Rewrote `docs/beads.md` so it now matches the current runtime instead of the older execution-loop/setup description: clarified approval semantics and downstream invalidation, documented the actual execution-setup plan/profile contract, added missing session-preservation and restart-recovery behavior, expanded the artifact/API inventory, and tightened the scheduler/finalization explanations. Also updated the directly related setup-plan wording in `docs/pre-implementation.md` to avoid conflicting schema descriptions.
