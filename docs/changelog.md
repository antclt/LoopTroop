...
- Fixed saved Kanban presets disappearing after a browser refresh. The UI-state provider now writes the next `looptroop-ui-state` synchronously during dispatch, mirrors `SET_PRESETS` updates to the relevant `looptroop-presets-*` key, merges those per-scope mirrors back on startup if the primary UI-state record is missing preset entries, normalizes malformed UI-state fields without discarding valid presets, and preserves stored preset scopes during unrelated stale UI-state writes.
