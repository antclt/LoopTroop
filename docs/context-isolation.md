...
| `execution_setup_profile` | Concrete runtime environment profile with reusable artifacts, tooling probes, and command families; setup-plan phases may receive it inline, coding reads `.ticket/runtime/execution-setup-profile.json` only when needed, and final testing reuses any declared wrapper automatically |
