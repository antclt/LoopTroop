...
For mobile, tablet, or another computer on the same trusted local network, use `npm run dev --lan`. LoopTroop binds the frontend and docs servers to the network, prints LAN URLs and a QR code, and still keeps backend/OpenCode control-plane ports on loopback so browser traffic goes through the normal Vite proxy. On WSL, LoopTroop automatically starts a Windows-side relay and prints the Windows LAN URL, because WSL `172.x` addresses are usually private to the Windows host.
