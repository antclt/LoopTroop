...
describe('Manual QA workspace checkpoints', () => {
  ...
  it('commits accepted final-test effects, quarantines residue, and records a clean baseline', () => {
    ...
    expect(result.quarantinedFiles).toEqual(['final-test.tmp'])
    ...
    expect(existsSync(resolve(setup.paths.ticketDir, 'manual-qa/v1/quarantine/final-test.tmp'))).toBe(true)
