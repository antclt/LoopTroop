...
export const PROM_EXECUTION_SETUP_PLAN: PromptTemplate = {
  ...
  instructions: [
    ...
    'Reproducible Inputs: Never propose copying ignored files from another checkout. A required file must be tracked or reproducibly generated by an approved repository-native command; otherwise list it as a readiness gap.',
...
export const PROM_EXECUTION_SETUP: PromptTemplate = {
  ...
  instructions: [
    ...
    'Reproducible Inputs: Never copy ignored files from the primary checkout. Generate required files only through approved repository-native commands; otherwise report the missing prerequisite with `checks.workspace = fail`.',
    'Quality Gate Policy: Default to bead test commands first, then impacted-or-package scoped lint/typecheck, and never block later phases on unrelated baseline debt.',
