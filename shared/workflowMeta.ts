...
const BASE_WORKFLOW_PHASES: WorkflowPhaseMeta[] = [
  ...
  },
  {
    ...
    description: 'AI coding agent executes beads one at a time with workflow-owned iteration timeouts that reset and retry in fresh sessions, while eligible OpenCode/provider stalls can preserve the active session for Continue. `done` means OpenCode success plus local finalization success, with required local commits for changed work, no-op completion support, push warnings, and retryable finalization failures.',
