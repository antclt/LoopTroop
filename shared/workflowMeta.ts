...
const WORKFLOW_PHASE_DETAILS = {
  ...
  WAITING_BEADS_APPROVAL: {
    overview: 'The final expanded beads plan is ready for human review before any coding begins. This is the last user-input gate before execution starts; once you approve, the coding agent will begin implementing beads one by one. You can review the full execution plan including task descriptions, dependencies, acceptance criteria, and test commands, edit the plan if needed, and optionally ask AI for one targeted extra fix at a time when coverage warnings still show unresolved PRD-to-beads gaps. Draft edits autosave with a visible last-save status, but they do not update the authoritative beads artifact until you explicitly Save.',
    steps: [
  ...
  WAITING_EXECUTION_SETUP_APPROVAL: {
    overview: 'LoopTroop audits the current workspace and pauses for your review before setup commands run. The plan separates temporary preparation from proof: ordered workspace probes must demonstrate that repository commands really work, while detected Git hooks and their resolved paths remain read-only evidence. The plan begins with the Git-hook policy frozen from the ticket, project, or profile choice, and you may review that policy plus add, edit, reorder, or remove every explicit validation command. Draft edits autosave with a visible last-save status, but they do not update the authoritative setup plan until you explicitly Save. Approval records the exact plan without a waiver or hidden hook command.',
    steps: [
      ...
      'Structured Setup Plan: The draft captures readiness, evidence, gaps, temporary steps and roots, ordered functional workspace probes, discovered project command families, quality-gate policy, the ticket-start-locked Git-hook policy and inheritance source, read-only detected hooks, and ordered editable validation commands. Manifests and version-only tool checks do not prove repository readiness. Unknown hooks remain visible without an invented command.',
      ...
      'User Review, Editing, And Draft Autosave: The approval UI lets you edit setup steps and workspace probes; select the hook policy; and add, edit, reorder, or remove explicit hook validation commands. Detected-hook evidence is not editable. Draft changes autosave between reloads, and the visible Draft autosave on indicator reports pending, saving, saved, conflict, or failure state plus the last server-acknowledged save time. Draft autosave does not update the authoritative setup plan; explicit Save applies the draft and records before/after hashes. An empty validation list is accepted without an extra warning, confirmation, waiver, or block, and archived versions remain read-only.',
      ...
      'Approval Handoff: Once approved, this plan locks the ticket-specific hook policy, detected evidence, exact validation-command order, workspace probes, and setup steps. Approval includes the serialized SHA-256 hash; stale hashes return 409. Runtime setup must execute this reviewed contract rather than silently substituting hook behavior or workspace files.',
    ],
    outputs: [
  ...
  PREPARING_EXECUTION_ENV: {
    overview: 'LoopTroop runs a dedicated execution setup phase after setup-plan approval and before coding. It materializes approved workspace inputs, verifies the approved readiness assessment, provisions missing runtime tooling only under approved temporary paths, validates wrappers and tooling probes, runs ordered repository-level workspace probes, and applies the approved Git-hook policy. Detected hooks remain evidence rather than hidden commit behavior: explicit validation commands run through the setup wrapper when selected, while LoopTroop-owned commits and pushes follow the locked policy. A workspace with repository command families is ready only after at least one functional repository probe succeeds; version-only checks cannot prove readiness. After automatic retries end, the live blocked view can send one user note to the preserved setup session for one additional manual attempt.',
    steps: [
  ...
  CODING: {
    overview: 'LoopTroop runs approved beads one at a time and treats the coding agent\'s `done/pass` marker as a candidate, not proof. The same session stays active while every declared bead test command runs sequentially through the approved setup wrapper inside the original iteration deadline. A failure receipt is returned to that session so the model can repair the work with the time remaining; only all-passing command receipts permit Git finalization. Deadline exhaustion and unresumable restart interruptions use the Ralph path: append a Failed Iteration Note, abandon or supersede the session, reset the worktree, and start a fresh bounded iteration with a new deadline. User retry guidance and local finalization failures append to their own histories. Finalization failures remain manual Retry/Cancel blocks and never trigger Ralph automatically.',
    steps: [
      ...
      'Inner Response Loop and Deterministic Verification: LoopTroop parses the `<BEAD_STATUS>` marker. Missing/malformed output uses the bounded structured-retry path; a non-passing marker asks the same session to continue. A valid `done/pass` marker starts independent verification instead of ending the session: declared `testCommands` run in order through the setup wrapper, stop at the first failure, and record command, duration, exit status, timeout state, and output excerpt. The failed receipt is sent back to the same session. Lint, typecheck, and qualitative checks remain self-reported unless their commands are explicitly declared. The single workflow deadline covers model work, marker repair, deterministic commands, and repair turns.',
      ...
      'Scoped Verification: Deterministic verification is intentionally language-agnostic and limited to the bead\'s declared `testCommands`; LoopTroop does not hardcode package managers, language runtimes, lint, typecheck, or qualitative commands. Each receipt is stored in the bead execution artifact and normal log. Missing tooling is reported as an environment failure instead of being worked around by copying ignored files from another checkout.',
      ...
      'Retry Budget and Finalization Boundary: Ralph failures consume the bead iteration budget and eventually block with `BEAD_RETRY_BUDGET_EXHAUSTED`. A successful deterministic verification sequence proceeds to local finalization. Commit/finalization failure instead appends only to `finalizationFailureNotes` and immediately enters manual Blocked Error recovery; it is not converted into another Ralph iteration. Retry with extra note... appends the user text unchanged only to `userRetryNotes` after safe recovery is proven.',
    ],
    outputs: [
  ...
  INTEGRATING_CHANGES: {
    overview: 'LoopTroop first applies the approved Git-hook policy, rerunning ordered explicit hook validation commands through the setup wrapper when `validate_explicitly` is selected and recording executed or skipped outcomes. It then turns bead commits into one clean candidate commit while preserving bead history and the final-test file-effects audit.',
    steps: [
      ...
      'Git-Hook Validation Gate: Under `validate_explicitly`, LoopTroop reruns the exact approved hook validation commands with timeouts, captured output, wrapper support, and tracked-file auditing before integration. Under `use_on_internal_commits`, Git runs hooks normally; under `ignore_internal_only`, hooks are bypassed and the skipped outcome is recorded.',
  ...
  BLOCKED_ERROR: {
    overview: 'A blocking failure interrupted the workflow and LoopTroop is waiting for a human decision before it can continue. The workspace summary names the phase that failed, shows its captured error, and explains the recovery actions currently available; historical occurrences are identified as read-only. The error is tied to the specific phase where the failure occurred, and the previous status is preserved so recovery knows exactly where to return. When available, persisted structured diagnostics expose underlying provider, model, session, timeout, OpenCode retry, output-truncation, and rate-limit-style failures alongside the human-readable error. Generic OpenCode provider errors are best-effort enriched from matching local OpenCode logs before display. You can inspect the error and choose Retry; Retry with extra note... for a live implementation or workspace setup block; Edit setup plan... for workspace setup; Continue when a provider interruption preserved an addressable session; or Cancel. Final-test local-only files are classified automatically and do not create a human recovery gate.',
    steps: [
      ...
      'Decision Point: You choose Retry, Retry with extra note..., Edit setup plan... when workspace setup failed, eligible same-session Continue, or Cancel. For coding, the note is appended to User Retry Notes after safe bead recovery. For workspace setup, only the note is sent to the preserved setup session for one manual attempt. Finalization failures already have their own concise Finalization Failure Note and remain manual Retry/Cancel blocks; Ralph is used only for iteration failures such as the shared coding/verification deadline.',
    ],
    outputs: [
...
const BASE_WORKFLOW_PHASES: WorkflowPhaseMeta[] = [
  ...
  },
  {
    ...
    description: 'Review and approve the full execution-ready beads plan with content-hash protection. Draft edits autosave with visible state and last-save time, but explicit Save is required to update the authoritative artifact before approval. Bead edits are limited to this gate, unresolved coverage gaps can be sent through optional one-click extra fixes, and approval records the reviewed JSONL hash before coding begins.',
  ...
  },
  {
    ...
    description: 'Review temporary setup, ordered functional workspace probes, detected Git hooks, the ticket-specific policy inherited and frozen at Start, and freely editable validation commands. Draft edits autosave with visible state and last-save time, but explicit Save is required to update the authoritative setup plan before approval. Approval content-hash locks the exact decision; detected evidence stays read-only and an empty validation list is accepted without a waiver.',
  ...
  },
  {
    ...
    description: 'Materializing approved workspace inputs and verifying readiness with wrappers, tooling probes, functional repository probes, and explicit Git-hook validation commands. After automatic retries end, the user can send one note to the preserved setup session for one additional manual attempt, or confirm a rewind to edit the setup plan.',
  ...
  },
  {
    ...
    description: 'Generates and runs whole-ticket tests, preserving explicit and tracked/staged changes while leaving untracked generated/cache/setup-local outputs usable on disk but outside totals, checkpoints, and delivery.',
  ...
  },
  {
    ...
    description: 'Reruns approved explicit Git-hook validation and exactly stages audited candidates into one clean commit; local-only outputs remain in the worktree and out of delivery.',
