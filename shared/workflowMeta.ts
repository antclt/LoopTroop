...
const WORKFLOW_PHASE_DETAILS = {
  ...
  CODING: {
    ...
    steps: [
      ...
      'Context Assembly: For the selected bead, LoopTroop assembles inline context from the bead\'s own description, acceptance criteria, file targets, and test commands (`bead_data`) plus accumulated context-wipe notes and any user guidance supplied through Retry with extra note (`bead_notes`). The prompt also points to the read-only setup profile at `.ticket/runtime/execution-setup-profile.json` for optional setup/tooling lookup. The agent receives only this bead-focused context — it does not see the full beads plan, other beads\' results, the PRD, or the interview.',
    ...
    outputs: [
      ...
      'Accumulated iteration notes in `bead.notes` for any bead that required context wipes, plus timestamped user guidance supplied through Retry with extra note — compact context for the next fresh attempt from Blocked Error.',
    ...
    notes: [
      ...
      'Context wipe notes accumulate: each failed iteration appends a new stamped note to `bead.notes`. Retry with extra note preserves that history and appends a separate `User retry note` entry with an ISO timestamp and the user\'s text unchanged. By iteration N the agent receives a progressive diagnostic history plus any user guidance — this is the primary mechanism for conveying failure context across iterations. LoopTroop-owned iteration timeouts consume bead attempts; OpenCode/provider interruptions may instead preserve an active session for Continue when diagnostics prove the session is resumable.',
  ...
  BLOCKED_ERROR: {
    ...
    steps: [
      ...
      'Decision Point: You choose Retry (which archives the failed phase attempt for every non-implementation status, creates a fresh attempt, returns the workflow to the previously blocked status, and re-attempts the failed operation), Retry with extra note for a live CODING block (which requires user text, safely resets the same failed or paused bead, appends the guidance to its existing notes, and starts a fresh attempt), Continue when available (which keeps the preserved OpenCode session and sends only `continue please` without versioning), Include in PR or Discard and Continue for unresolved final-test-produced files, or Cancel (which moves the ticket to the terminal Canceled state, preserving all artifacts). CODING is the implementation exception and keeps its failed-bead reset/retry history. Workflow-owned iteration timeouts consume bead attempts and do not expose Continue; eligible OpenCode/provider retry-budget or session interruptions can re-enter the preserved session through Continue.',
    ],
    outputs: [
    ...
    notes: [
      ...
      'Retry with extra note is shown only on the live Blocked Error view when `previousStatus` is CODING. It requires 1–20,000 characters after rejecting whitespace-only input, preserves all prior notes, and appends the user\'s text unchanged beneath a `User retry note` label and ISO timestamp. Historical errors and blocks from other phases do not offer it.',
