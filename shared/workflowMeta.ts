...
const WORKFLOW_PHASE_DETAILS = {
  ...
  PREPARING_EXECUTION_ENV: {
    ...
    notes: [
      ...
      'Everything created here is temporary runtime state. Cleanup removes temp roots at ticket end while preserving audit artifacts and execution logs. Later internal Git operations use the approved hook policy and ignore setup roots so prepared toolchains cannot become implementation work. LoopTroop never copies ignored files from the primary checkout to make a probe pass; missing prerequisites must come from an approved repository-native command or remain a visible setup failure.',
  ...
  BLOCKED_ERROR: {
    ...
    steps: [
      ...
      'Decision Point: You choose Retry, implementation-only Retry with extra note, eligible same-session Continue, a final-test file-effects decision, or Cancel. Retry with extra note appends only to User Retry Notes after safe bead recovery. Finalization failures already have their own concise Finalization Failure Note and remain manual Retry/Cancel blocks; Ralph is used only for iteration failures such as the shared coding/verification deadline.',
    ],
    outputs: [
      ...
      'Retry, implementation-only Retry with extra note, eligible same-session Continue, final-test file-effect Include in PR / Discard and Continue, or cancel decision point for manual intervention, with non-implementation retry artifacts/logs separated by phase attempt.',
    ],
    transitions: [
    ...
    notes: [
      ...
      'Retry with extra note is shown only on the live Blocked Error view when `previousStatus` is CODING. It requires 1–20,000 characters after rejecting whitespace-only input and appends the user\'s text unchanged as a structured User Retry Notes entry. Failed Iteration Notes and Finalization Failure Notes are separately append-only, ANSI-free machine histories. Historical errors and blocks from other phases do not offer note-bearing retry.',
...
const BASE_WORKFLOW_PHASES: WorkflowPhaseMeta[] = [
  ...
  },
  {
    ...
    description: 'Verifying readiness with approved wrappers, tooling probes, functional repository probes, and explicit Git-hook validation commands; recording detected hooks and command receipts; and refusing to substitute ignored files from another checkout. Users may rewind to edit or regenerate the setup plan before coding begins.',
  ...
  },
  {
    ...
    description: 'Reruns approved explicit Git-hook validation when selected, records executed or skipped outcomes, and squashes individual bead commits into one clean candidate commit while preserving the audit trail.',
  ...
  },
  {
    id: 'BLOCKED_ERROR',
    label: 'Error (reason)',
    description: 'A workflow phase failed and needs a recovery decision. The workspace summary identifies the failed phase, shows the captured error, and explains what the currently available recovery actions will do.',
