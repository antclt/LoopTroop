...
const WORKFLOW_PHASE_DETAILS = {
  ...
  PREPARING_EXECUTION_ENV: {
    ...
    outputs: [
      'Canonical execution setup profile artifact describing reusable temp roots, prepared runtime wrappers, tooling probe commands, failed-tooling evidence when applicable, discovered command families, and quality-gate policy for later coding beads and final testing.',
  ...
  CODING: {
    ...
    steps: [
      ...
      'Success Path — Local Finalization, Diff Capture, Artifacts, and Broadcast: When the inner loop exits successfully, LoopTroop first persists the full execution result as a `bead_execution:{beadId}` checkpoint tied to the exact in-progress bead state. It then finalizes the local work: `commitBeadChanges` must create a local per-bead commit when Git-visible project changes exist in any language or file extension, excluding LoopTroop/setup roots and untracked generated/local noise. A true no-op may complete without a commit, and a remote push failure after a successful local commit is logged as a warning. Only after local finalization succeeds does LoopTroop mark the bead `done`, update progress counters, capture a code-only `bead_diff:{beadId}` from `beadStartCommit` when available, and broadcast `bead_complete` with progress counters.',
...
const BASE_WORKFLOW_PHASES: WorkflowPhaseMeta[] = [
  ...
  },
  {
    ...
    description: 'Verifying readiness, provisioning missing required runtime tooling under ticket-owned temp roots, validating declared wrappers/tooling probes, and recording failed-launcher evidence before coding begins. Ready setup results are blocked if probes fail, wrappers are unusable, or committable project changes are left behind.',
