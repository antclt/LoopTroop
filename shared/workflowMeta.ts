...
const WORKFLOW_PHASE_DETAILS = {
  ...
  PREPARING_EXECUTION_ENV: {
    ...
    notes: [
      ...
      'Everything created here is temporary runtime state. Cleanup removes the temp roots at ticket end while preserving audit artifacts and the execution log. Later bead commits ignore setup roots recorded in the profile so prepared toolchains cannot become implementation work.',
  ...
  BLOCKED_ERROR: {
    overview: 'A blocking failure interrupted the workflow and LoopTroop is waiting for a human decision before it can continue. The error is tied to the specific phase where the failure occurred, and the previous status is preserved so recovery knows exactly where to return. When available, persisted structured diagnostics expose underlying provider, model, session, timeout, OpenCode retry, output-truncation, and rate-limit-style failures alongside the human-readable error. You can see the error details, inspect logs around the failing moment, and choose Retry, Continue when a preserved OpenCode session is resumable, or Cancel.',
    steps: [
      'Error Recording: LoopTroop captures the error message, error codes (if available), the precise timestamp of the failure, and the workflow status where the failure occurred. Provider, model, session, timeout, OpenCode retry, output-truncation, and rate-limit-style diagnostics are persisted as structured fields when the failing subsystem exposes them. If structured-output validation later fails because OpenCode returned no usable content or stopped with an output-length finish reason, the latest underlying OpenCode failure is preserved instead of replacing it with only the parser wrapper. This information is stored as an error occurrence record.',
    ...
    outputs: [
      'Error occurrence history with timestamps, error messages, error codes, structured provider/model/session/timeout/OpenCode retry/output-truncation/rate-limit diagnostics when available, and the phase where each failure occurred.',
...
const BASE_WORKFLOW_PHASES: WorkflowPhaseMeta[] = [
  ...
  },
  {
    ...
    description: 'AI coding agent executes beads one at a time; each bead has its own session, context-wipe recovery, concise internal CMD summaries, and a git commit after success that excludes LoopTroop runtime and setup-cache roots.',
