...
const WORKFLOW_PHASE_DETAILS = {
  ...
  WAITING_EXECUTION_SETUP_APPROVAL: {
    ...
    steps: [
      ...
      'User Review And Editing: The approval UI lets you edit setup steps and workspace probes; select the hook policy; and add, edit, reorder, or remove explicit hook validation commands. Detected-hook evidence is not editable. An empty validation list is accepted without an extra warning, confirmation, waiver, or block. Manual saves record before/after hashes and archived versions remain read-only.',
...
const BASE_WORKFLOW_PHASES: WorkflowPhaseMeta[] = [
  ...
  },
  {
    ...
    description: 'Review temporary setup, ordered functional workspace probes, detected Git hooks, the ticket-specific policy inherited and frozen at Start, and freely editable validation commands. Approval content-hash locks the exact decision; detected evidence stays read-only and an empty validation list is accepted without a waiver.',
