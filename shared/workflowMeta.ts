...
const WORKFLOW_PHASE_DETAILS = {
  ...
  WAITING_EXECUTION_SETUP_APPROVAL: {
    ...
    steps: [
      ...
      'Structured Setup Plan: The draft captures readiness, evidence, gaps, temporary steps and roots, ordered functional workspace probes, discovered project command families, quality-gate policy, the resolved Git-hook policy, read-only detected hooks, and ordered editable validation commands. Manifests and version-only tool checks do not prove repository readiness. Unknown hooks remain visible without an invented command.',
...
const BASE_WORKFLOW_PHASES: WorkflowPhaseMeta[] = [
  ...
  },
  {
    ...
    description: 'Review temporary setup, ordered functional workspace probes, detected Git hooks, the ticket-specific hook policy, and freely editable validation commands. Approval content-hash locks the exact decision; detected evidence stays read-only and an empty validation list is accepted without a waiver.',
